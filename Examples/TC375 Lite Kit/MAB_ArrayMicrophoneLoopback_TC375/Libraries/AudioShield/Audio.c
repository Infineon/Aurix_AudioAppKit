...
int16_t __attribute__ ((__align(4*2*64))) MicBuf[4][64];
...
void I2S_Init_DMA(void) {
    ...
    /*
     * inits the GTM together with QSPI (Slave-Mode) to make a I2S master-interface - additionally the PDM demodulators are activated.
     *
     ...
     * P00.0 -> GTM_TOUT9 / TOM0CH8 -> 3.072 MHz PDM CLK output
     *
     *
     ...
     *
     *
     * The 4 microphones are connected to EDSADC modules no. 0,1,2 and 5 with following input pins
     *
     * EDSADC0 - CLKIN P0.11 - DATAIN P0.12
     * EDSADC1 - CLKIN P0.9 - DATAIN P0.10
     * EDSADC2 - CLKIN P33.1 - DATAIN P33.2
     * EDSADC5 - CLKIN P0.1 - DATAIN P0.2
     *
     */
    ...
    P00_PDR0.B.PD0 = 0x0; //PDM CLK bottom
    ...
    //Demodulator Input Config
    //Write Control for Mode Settings=1 (Bit-Offset 31)
    //Set external clock Input B for EDSADC2 and external Input A for EDSADC 0,1+5 (Bit Offset 8)
    //Set external data Input B for EDSADC2 and external Input A for EDSADC 0,1+5 (Bit Offset 4)
    //Use rising clock edge for modulator input (Bit Offset 0)
    //Write Control for Data Stream Selection (Bit Offset 15)
    //Data Read Mode: Double (Bit Offset 26)
    //Integrator No Trigger (Bit Offset 20)
    EDSADC_DICFG2.U =  (1<<31)  | (4<<8) | (2<<4) | (5) | (1<<15) | (2<<26) | (3<<20);
    EDSADC_DICFG0.U =  (1<<31)  | (3<<8) | (0<<4) | (5) | (1<<15) | (2<<26) | (3<<20);
    EDSADC_DICFG1.U =  (1<<31)  | (3<<8) | (0<<4) | (5) | (1<<15) | (2<<26) | (3<<20);
    EDSADC_DICFG5.U =  (1<<31)  | (3<<8) | (0<<4) | (5) | (1<<15) | (2<<26) | (3<<20);

    //Set decimation to 64
    EDSADC_CH2_FCFGC.B.CFMDF = 63;
    EDSADC_CH0_FCFGC.B.CFMDF = 63;
    EDSADC_CH1_FCFGC.B.CFMDF = 63;
    EDSADC_CH5_FCFGC.B.CFMDF = 63;

    //set demodulator active
    EDSADC_GLOBRC.B.CH2RUN = 1;
    EDSADC_GLOBRC.B.CH0RUN = 1;
    EDSADC_GLOBRC.B.CH1RUN = 1;
    EDSADC_GLOBRC.B.CH5RUN = 1;

    //Configure CIC Filter Settings
    EDSADC_GAINCORR2.B.CICSHIFT = 15;
    EDSADC_GAINCTR2.B.CICSHIFT = 15;
    EDSADC_GAINCORR0.B.CICSHIFT = 15;
    EDSADC_GAINCTR0.B.CICSHIFT = 15;
    EDSADC_GAINCORR1.B.CICSHIFT = 15;
    EDSADC_GAINCTR1.B.CICSHIFT = 15;
    EDSADC_GAINCORR5.B.CICSHIFT = 15;
    EDSADC_GAINCTR5.B.CICSHIFT = 15;

    //Generate DMA trigger when FIFO level reaches 2
    EDSADC_RFC2.B.SRLVL = 1;
    EDSADC_RFC0.B.SRLVL = 1;
    EDSADC_RFC1.B.SRLVL = 1;
    EDSADC_RFC5.B.SRLVL = 1;

    //Activate FIFO output trigger when configured FIFO level is reached
    EDSADC_FCFGM2.U = (1<<31) | (1<<15) | (3<<16);
    EDSADC_FCFGM0.U = (1<<31) | (1<<15) | (3<<16);
    EDSADC_FCFGM1.U = (1<<31) | (1<<15) | (3<<16);
    EDSADC_FCFGM5.U = (1<<31) | (1<<15) | (3<<16);


    //Route EDSADC sample-available Trigger Signals to DMA Channel 45,46,47,48
    SRC_DSADCSRM2.B.SRPN = 48;
    SRC_DSADCSRM2.B.TOS = 1;
    SRC_DSADCSRM2.B.SRE = 1;

    SRC_DSADCSRM0.B.SRPN = 47;
    SRC_DSADCSRM0.B.TOS = 1;
    SRC_DSADCSRM0.B.SRE = 1;

    SRC_DSADCSRM1.B.SRPN = 46;
    SRC_DSADCSRM1.B.TOS = 1;
    SRC_DSADCSRM1.B.SRE = 1;

    SRC_DSADCSRM5.B.SRPN = 45;
    SRC_DSADCSRM5.B.TOS = 1;
    SRC_DSADCSRM5.B.SRE = 1;
    ...
    //Microphone DMA
    //Hint: The EDSADC is configured that always two 16-bit samples can be read by one single 32-bit read access.
    //Therefore the DMA operates in 32-Bit mode to reduce the bus-access load
    //So the 2*32 word 16-bit buffer (as initialized with int16_t) is actually handled as a 2*16 word 32-bit buffer

    //Init DMA for Mic 0

    //define rx dma source
    DMA_SADR048.U           = (uint32) &EDSADC_RESM2.U;
    DMA_ADICR048.B.SCBE     = 1; /* Enable source circular buffer */
    DMA_ADICR048.B.CBLS     = 0; /* Source address SADR[31:0] is not updated ie same source address will be used for all transfers */
    DMA_ADICR048.B.WRPSE    = 0; /* Disable wrap source buffer interrupt */

    //define rx dma destination
    DMA_ADICR048.B.SHCT     = 0xb; /* Shadow control: Destination double-buffering with automatic switch */
    DMA_DADR048.U           = (uint32) &MicBuf[0][0]; //address of first buffer half
    DMA_SHADR048.U          = (uint32) &MicBuf[0][32]; //address of second buffer half
    DMA_ADICR048.B.DMF      = 0; /* Destination address modified by 1x CHDW */
    DMA_ADICR048.B.INCD     = 1; /* Increment destination address automatically */
    DMA_ADICR048.B.DCBE     = 1; /* Enable destination circular buffer */
    DMA_ADICR048.B.WRPDE    = 1; /* Enable wrap destination buffer interrupt */
    DMA_ADICR048.B.CBLD     = 6; /* 16 (32-bit) words per single buffer */

    DMA_ADICR048.B.STAMP    = 0; /* Disable timestamp */


    DMA_CHCFGR048.B.TREL    = 16; /* Transfer count reload value: 16 words per buffer - if interpreted as 32-Bit buffer. Actually it is 32*16Bit */
    DMA_CHCFGR048.B.BLKM    = 0;  /* 1 dma move executed per transfer */
    DMA_CHCFGR048.B.RROAT   = 0;  /* One transfer per trigger */
    DMA_CHCFGR048.B.CHMODE  = 1;  /* Cont. Transaction Mode */
    DMA_CHCFGR048.B.CHDW    = 2;  /* Channel Data Width is 32 bit */
    DMA_CHCFGR048.B.PATSEL  = 0;  /* Pattern detection disabled */
    DMA_CHCFGR048.B.PRSEL   = 0;  /* Peripheral trigger selected */

    DMA_TSR048.B.ECH        = 1; /* Enable Hardware Channel Requests */
    ...
    //define rx dma source
    DMA_SADR047.U           = (uint32) &EDSADC_RESM0.U;
    DMA_ADICR047.B.SCBE     = 1; /* Enable source circular buffer */
    DMA_ADICR047.B.CBLS     = 0; /* Source address SADR[31:0] is not updated ie same source address will be used for all transfers */
    DMA_ADICR047.B.WRPSE    = 0; /* Disable wrap source buffer interrupt */

    //define rx dma destination
    DMA_ADICR047.B.SHCT     = 0xb; /* Shadow control: Destination double-buffering with automatic switch */
    DMA_DADR047.U           = (uint32) &MicBuf[1][0]; //address of first buffer half
    DMA_SHADR047.U          = (uint32) &MicBuf[1][32]; //address of second buffer half
    DMA_ADICR047.B.DMF      = 0; /* Destination address modified by 1x CHDW */
    DMA_ADICR047.B.INCD     = 1; /* Increment destination address automatically */
    DMA_ADICR047.B.DCBE     = 1; /* Enable destination circular buffer */
    DMA_ADICR047.B.WRPDE    = 1; /* Enable wrap destination buffer interrupt */
    DMA_ADICR047.B.CBLD     = 6; /* 16 (32-bit) words per single buffer */

    DMA_ADICR047.B.STAMP    = 0; /* Disable timestamp */


    DMA_CHCFGR047.B.TREL    = 16; /* Transfer count reload value: 16 words per buffer - if interpreted as 32-Bit buffer. Actually it is 32*16Bit */
    DMA_CHCFGR047.B.BLKM    = 0;  /* 1 dma move executed per transfer */
    DMA_CHCFGR047.B.RROAT   = 0;  /* One transfer per trigger */
    DMA_CHCFGR047.B.CHMODE  = 1;  /* Cont. Transaction Mode */
    DMA_CHCFGR047.B.CHDW    = 2;  /* Channel Data Width is 32 bit */
    DMA_CHCFGR047.B.PATSEL  = 0;  /* Pattern detection disabled */
    DMA_CHCFGR047.B.PRSEL   = 0;  /* Peripheral trigger selected */

    DMA_TSR047.B.ECH        = 1; /* Enable Hardware Channel Requests */


    //Init DMA for Mic 2

    //define rx dma source
    DMA_SADR046.U           = (uint32) &EDSADC_RESM1.U;
    DMA_ADICR046.B.SCBE     = 1; /* Enable source circular buffer */
    DMA_ADICR046.B.CBLS     = 0; /* Source address SADR[31:0] is not updated ie same source address will be used for all transfers */
    DMA_ADICR046.B.WRPSE    = 0; /* Disable wrap source buffer interrupt */

    //define rx dma destination
    DMA_ADICR046.B.SHCT     = 0xb; /* Shadow control: Destination double-buffering with automatic switch */
    DMA_DADR046.U           = (uint32) &MicBuf[2][0]; //address of first buffer half
    DMA_SHADR046.U          = (uint32) &MicBuf[2][32]; //address of second buffer half
    DMA_ADICR046.B.DMF      = 0; /* Destination address modified by 1x CHDW */
    DMA_ADICR046.B.INCD     = 1; /* Increment destination address automatically */
    DMA_ADICR046.B.DCBE     = 1; /* Enable destination circular buffer */
    DMA_ADICR046.B.WRPDE    = 1; /* Enable wrap destination buffer interrupt */
    DMA_ADICR046.B.CBLD     = 6; /* 16 (32-bit) words per single buffer */

    DMA_ADICR046.B.STAMP    = 0; /* Disable timestamp */


    DMA_CHCFGR046.B.TREL    = 16; /* Transfer count reload value: 16 words per buffer - if interpreted as 32-Bit buffer. Actually it is 32*16Bit */
    DMA_CHCFGR046.B.BLKM    = 0;  /* 1 dma move executed per transfer */
    DMA_CHCFGR046.B.RROAT   = 0;  /* One transfer per trigger */
    DMA_CHCFGR046.B.CHMODE  = 1;  /* Cont. Transaction Mode */
    DMA_CHCFGR046.B.CHDW    = 2;  /* Channel Data Width is 32 bit */
    DMA_CHCFGR046.B.PATSEL  = 0;  /* Pattern detection disabled */
    DMA_CHCFGR046.B.PRSEL   = 0;  /* Peripheral trigger selected */

    DMA_TSR046.B.ECH        = 1; /* Enable Hardware Channel Requests */


    //Init DMA for Mic 3

    //define rx dma source
    DMA_SADR045.U           = (uint32) &EDSADC_RESM5.U;
    DMA_ADICR045.B.SCBE     = 1; /* Enable source circular buffer */
    DMA_ADICR045.B.CBLS     = 0; /* Source address SADR[31:0] is not updated ie same source address will be used for all transfers */
    DMA_ADICR045.B.WRPSE    = 0; /* Disable wrap source buffer interrupt */

    //define rx dma destination
    DMA_ADICR045.B.SHCT     = 0xb; /* Shadow control: Destination double-buffering with automatic switch */
    DMA_DADR045.U           = (uint32) &MicBuf[3][0]; //address of first buffer half
    DMA_SHADR045.U          = (uint32) &MicBuf[3][32]; //address of second buffer half
    DMA_ADICR045.B.DMF      = 0; /* Destination address modified by 1x CHDW */
    DMA_ADICR045.B.INCD     = 1; /* Increment destination address automatically */
    DMA_ADICR045.B.DCBE     = 1; /* Enable destination circular buffer */
    DMA_ADICR045.B.WRPDE    = 1; /* Enable wrap destination buffer interrupt */
    DMA_ADICR045.B.CBLD     = 6; /* 16 (32-bit) words per single buffer */

    DMA_ADICR045.B.STAMP    = 0; /* Disable timestamp */


    DMA_CHCFGR045.B.TREL    = 16; /* Transfer count reload value: 16 words per buffer - if interpreted as 32-Bit buffer. Actually it is 32*16Bit */
    DMA_CHCFGR045.B.BLKM    = 0;  /* 1 dma move executed per transfer */
    DMA_CHCFGR045.B.RROAT   = 0;  /* One transfer per trigger */
    DMA_CHCFGR045.B.CHMODE  = 1;  /* Cont. Transaction Mode */
    DMA_CHCFGR045.B.CHDW    = 2;  /* Channel Data Width is 32 bit */
    DMA_CHCFGR045.B.PATSEL  = 0;  /* Pattern detection disabled */
    DMA_CHCFGR045.B.PRSEL   = 0;  /* Peripheral trigger selected */

    DMA_TSR045.B.ECH        = 1; /* Enable Hardware Channel Requests */
    ...
    /*
     * Configure TOM0CH8 which is PDMCLK -> divide CLKINPUT/8 -> 24.576/8 = 3.072 MHz
     * Clocking derived from TRIG_(x) chain
     */
    GTM_TOM0_CH8_CN0.U = 0;
    GTM_TOM0_CH8_SR0.U = 8;
    GTM_TOM0_CH8_SR1.U = 4;
    GTM_TOM0_CH8_CM0.U = GTM_TOM0_CH8_SR0.U;
    GTM_TOM0_CH8_CM1.U = GTM_TOM0_CH8_SR1.U;
    GTM_TOM0_CH8_CTRL.U = 0;
    GTM_TOM0_CH8_CTRL.B.CLK_SRC_SR = 5;
    GTM_TOM0_CH8_CTRL.B.ECLK_SRC = 1;
    GTM_TOM0_CH8_CTRL.B.SL = 1;
    GTM_TOM0_CH8_CTRL.B.OSM = 0;
    GTM_TOM0_TGC1_OUTEN_CTRL.B.OUTEN_CTRL0 = 0x2;
    GTM_TOM0_TGC1_ENDIS_CTRL.B.ENDIS_CTRL0 = 2;
    GTM_TOM0_TGC1_FUPD_CTRL.B.FUPD_CTRL0 = 2;
    GTM_TOM0_TGC1_GLB_CTRL.B.UPEN_CTRL0 = 2;
