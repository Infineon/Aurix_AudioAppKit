...
static IfxI2c_I2c_Device i2c_Dev;
/*****************************************************************************/
/*----------------------------Function Prototypes----------------------------*/
/*****************************************************************************/
...
void Init_A2B_Codec(void) {
    ...
    i2cDeviceConfig.deviceAddress = 0x00<<1;  // 8 bit device address


    // initialize the i2c device handle
    IfxI2c_I2c_initDevice(&i2c_Dev, &i2cDeviceConfig);
    ...
    for (int i=0; i<CONFIG_LEN; i++) {
        ...
        //write config
            ...
            if (gaA2BConfig[i].nAddrWidth==1) {
                ...
                //read dev address
                i2c_Dev.deviceAddress = gaA2BConfig[i].nDeviceAddr<<1;
                ...
                //transmit
                while(IfxI2c_I2c_write(&i2c_Dev, i2c_data_buf, gaA2BConfig[i].nDataCount+1) == IfxI2c_I2c_Status_nak);
            }
            ...
            if (gaA2BConfig[i].nAddrWidth==2) {
                ...
                //transmit
                while(IfxI2c_I2c_write(&i2c_Dev, i2c_data_buf, gaA2BConfig[i].nDataCount+2) == IfxI2c_I2c_Status_nak);
            }
        }
