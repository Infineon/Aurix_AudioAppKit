...
//decide if we use Audio-Input from A²B network (=1) or from Line-Input from Codec (=0)
uint8_t A2B_LineIn_Select = 1;
...
static __attribute__((__noreturn__)) void OsTask_AudioProcess(void *arg)
{
    ...
    Set_TDM_DataIn_Mux(A2B_LineIn_Select);
    ...
    while (1)
    {
        ...
        for (int i=0+BufferOffset; i<2048+BufferOffset; i=i+8) {
            ...
            //do something with the samples (e.g. sum up and apply volume control
            if (A2B_LineIn_Select==1) {
                lSample = (TDM_IN[0] + TDM_IN[2] + TDM_IN[4] + TDM_IN[6])*volume;
                rSample = (TDM_IN[1] + TDM_IN[3] + TDM_IN[5] + TDM_IN[7])*volume;
            }

            else {
                //Stereo Audio-ADC is on Slot 0+4
                lSample = TDM_IN[0]*volume;
                rSample = TDM_IN[4]*volume;
            }
    ...
    while (1) {
        ...
        if (CheckButtonRuntime()==1) {
            if (A2B_LineIn_Select == 1) {
                ...
                //change to Line-In Signal; LED on
                A2B_LineIn_Select = 0;
                P00_OUT.B.P5 = 0;
            }
