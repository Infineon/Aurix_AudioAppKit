...
#define I2C_ADDR_CODEC  0x0a
#define I2C_ADDR_PLL    0x4e
...
void I2CWriteCS2x00(uint8_t address, uint8_t data) {
    ...
    txbuf[0] = address;
    txbuf[1] = data;
    I2C0_Write(I2C_ADDR_PLL, txbuf, size);
}
...
void I2CReadCS2x00(uint8_t address, uint8_t *data) {
    uint8_t datarx = address;
    I2C0_Read(I2C_ADDR_PLL, &datarx, 1, 1);
    *data = datarx;
}
...
void I2CWriteSGTL5000(uint16_t address, uint16_t data) {
    ...
    txbuf[2] = (data>>8)&0xff;
    txbuf[3] = data&0xff;
    I2C0_Write(I2C_ADDR_CODEC, txbuf, size);
}
...
void I2S_Init_DMA(void) {
    ...
    /*
     * inits the GTM together with QSPI (Slave-Mode) to make a I2S master-interface for interfacing Stereo-PDM microphones
     *
     ...
     * P10.2 -> GTM_TOUT104 / TOM0CH2 -> 6.144 MHz for clocking QSPI module in double-speed mode
    ...
    /*
     * Configure TOM0CH2 which is double-speed QSPI clock -> divide CLKINPUT/4 -> 24.576/4 = 6.144 MHz with offset of 2
     * Clocking derived from TRIG_(x) chain
     */
    GTM_TOM0_CH2_CN0.U = 2;
    GTM_TOM0_CH2_SR0.U = 4;
    GTM_TOM0_CH2_SR1.U = 2;
