...
void Open_PDM_Filter_Init(TPDMFilter_InitStruct *Param)
{
  ...
  Param->OldOut2 = Param->OldIn2 = Param->OldZ2 = 0;
...
void Open_PDM_Filter_64(uint8_t* data0, uint8_t* data1, int16_t* dataOut0, int16_t* dataOut1, uint16_t volume, TPDMFilter_InitStruct *Param)
{
  ...
  int64_t Z, ZX, Z0, Z1, Z2, Z3, Z4, Z5;
  ...
  for (i = 0, data_out_index = 0; i < 16; i++, data_out_index += channels) {
  #ifdef USE_LUT
    Z0 = filter_tables_64[j](data0, 0);
    Z1 = filter_tables_64[j](data0, 1);
    Z2 = filter_tables_64[j](data0, 2);
    Z3 = filter_tables_64[j](data1, 0);
    Z4 = filter_tables_64[j](data1, 1);
    Z5 = filter_tables_64[j](data1, 2);
    ...
    ZX = Param->Coef[4] + Z5 - sub_const;
    Param->Coef[4] = Param->Coef[3] + Z4;
    Param->Coef[3] = Z3;
    ...
    OldOut2 = (Param->HP_ALFA * (OldOut2 + ZX - OldIn2)) >> 8;
    OldIn2 = ZX;
    OldZ2 = ((256 - Param->LP_ALFA) * OldZ2 + Param->LP_ALFA * OldOut2) >> 8;
    ...
    ZX = OldZ2 * volume;
    ZX = RoundDiv(ZX, div_const);
    ZX = SaturaLH(ZX, -32700, 32700);

    dataOut0[data_out_index] = Z;
    dataOut1[data_out_index] = ZX;
    data0 += data_inc;
    data1 += data_inc;
  }
