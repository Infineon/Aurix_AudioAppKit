...
//decide if I2S-clocking should run from fixed oscillator or from PLL.
//With fixed oscillator, only 48 kHz sample-rate is possible
uint8_t usePLL = 1;
...
static __attribute__((__noreturn__)) void OsTask_AudioProcess(void *arg)
{
    ...
    //Set Ref-Clock mux
    InitRefClockMux(usePLL);

    if (usePLL) {
        //Init PLL for 44.1 kHz sample-rate operation (currently possible is 44100, 48000, 88200 and 96000)
        InitCS2x00(44100);
        //wait for PLL lock
        while (IsPllLock()==0);
    }
    ...
    while (1)
    {
        ...
        for (int i=0+BufferOffset; i<64+BufferOffset; i=i+2) {
            lSample = (float) rxBuf[i];
            rSample = (float) rxBuf[i+1];
